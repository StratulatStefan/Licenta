...
public class FrontendProxyManager {
            ...
            public void run() {
                try {
                    ...
                    if(operation == NewFileRequest.class){
                        ...
                        FileSenderService.sendFile(requestData.getUserId(), token, requestData.getFilename(), requestData.getDescription());
                        long timeElapsed = System.currentTimeMillis() - start;
                        long fileCRC = FileSystem.calculateCRC(requestData.getFilename());
                        FileSenderService.waitForFeedback(requestData.getUserId(), token, requestData.getFilename(), timeElapsed, fileCRC);
                    }
                    else if(operation == DeleteFileRequest.class){
